...
- Figma: remote MCP connected in Devin CLI at user scope (`figma`, `https://mcp.figma.com/mcp`). Team `DFDL Studio`, Pro tier, Full seat. All files in that team. Agents write to the scratch file only. Do not add Figma's desktop server (`127.0.0.1:3845`) to the CLI; it shadows the remote write tools.
...
| 1 | Color and surfaces lab: neutral ramp with roles, three accents at matched OKLCH L/C shown in context, three shadow recipes; accent revisited after dark-mode shadows. Baseline = dylanfdl.com tokens | ~45 min | `tokens.css` colors | built, awaiting Dylan's pick |
