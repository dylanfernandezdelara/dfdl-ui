...
export function PopoverSpec({ open: controlled }: Controlled) {
  ...
  return (
    <div className="relative inline-block">
      ...
      <div role="menu" data-state={open ? "open" : "closed"} className="spec-popover absolute top-full left-0 z-10 mt-1 w-52 rounded-md bg-raised p-1 elevation-floating">
        {[
          ["Rename", Pencil],
          ["Fork from here", GitFork],
          ["Regenerate", RotateCw],
        ].map(([label, Icon]) => (
          <button key={label as string} type="button" role="menuitem" onClick={() => set(false)} className="flex h-control w-full items-center gap-2 rounded-xs px-2 text-ui text-fg hover:bg-surface-hover">
        ...
        <button type="button" role="menuitem" onClick={() => set(false)} className="flex h-control w-full items-center gap-2 rounded-xs px-2 text-ui text-danger-text hover:bg-danger-bg">
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(control, "w-7 border border-line bg-surface px-0 text-fg-secondary elevation-raised hover:bg-surface-hover hover:text-fg")}
      >
  ...
  return (
    <div className="relative h-56 overflow-hidden rounded-lg border border-line bg-page">
      <div className="p-major">
        ...
        <button type="button" onClick={toggle} className={cn(control, "mt-minor border border-line bg-surface text-fg elevation-raised hover:bg-surface-hover")}>
          Delete workspace
        </button>
      </div>
      ...
      <div role="dialog" aria-modal="true" data-state={open ? "open" : "closed"} className="spec-dialog absolute inset-x-8 top-1/2 -translate-y-1/2 rounded-lg bg-raised p-major elevation-floating">
        ...
        <p className="mt-1 text-ui text-fg-secondary">All 128 chats and their forks go with it. This cannot be undone.</p>
        <div className="mt-minor flex justify-end gap-2">
          <button type="button" onClick={() => set(false)} className={cn(control, "border border-line bg-surface text-fg hover:bg-surface-hover")}>Cancel</button>
  ...
  return (
    <div className="relative h-72 w-44 overflow-hidden rounded-xl border border-line bg-page">
      <div className="p-minor">
        ...
        <button type="button" onClick={toggle} className={cn(control, "mt-minor w-full border border-line bg-surface text-fg elevation-raised hover:bg-surface-hover")}>
          Model
        </button>
      </div>
      <div data-state={open ? "open" : "closed"} className="spec-backdrop absolute inset-0 bg-overlay" onClick={() => set(false)} />
      <div role="dialog" data-state={open ? "open" : "closed"} className="spec-drawer absolute inset-x-0 bottom-0 rounded-t-lg bg-raised p-minor elevation-floating">
        <div className="mx-auto mb-minor h-1 w-8 rounded-full bg-line-strong" />
        {["Muse Spark 1.3", "GPT-5.6 Luna"].map((m, i) => (
          <button key={m} type="button" onClick={() => set(false)} className="flex h-control w-full items-center justify-between rounded-xs px-2 text-ui text-fg hover:bg-surface-hover">
...
export function TabsSpec() {
  ...
  return (
    <div className="relative flex gap-1 border-b border-separator">
      {tabs.map((t, i) => (
        <button
          key={t}
          ref={(el) => {
          ...
          onClick={() => setActive(i)}
          className={cn("h-control px-2 text-ui transition-interactive duration-fast ease-out", i === active ? "text-fg-strong" : "text-fg-secondary hover:text-fg")}
        >
          {t}
        </button>
      ))}
      {/* Rendered only once measured, so the first paint does not animate the bar in from zero. */}
      {ind ? <span aria-hidden className="spec-tab-indicator absolute bottom-0 left-0 h-0.5 rounded-full bg-fg-strong" style={{ "--tab-x": `${ind.x}px`, "--tab-w": `${ind.w}px` } as React.CSSProperties} /> : null}
    </div>
  )
}
...
export function SwitchSpec() {
  ...
  return (
    <label className="flex items-center gap-2 text-ui text-fg">
      ...
      >
        <span className={cn("spec-switch-thumb absolute top-0.5 left-0.5 size-4 rounded-full bg-raised elevation-raised", on && "translate-x-4")} />
      </button>
      Web search
    </label>
  )
