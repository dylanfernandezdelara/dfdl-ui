    ...
    <div className="flex min-h-full flex-col">
      <header className="sticky top-0 z-20 flex h-12 items-center justify-between bg-page/90 px-major backdrop-blur hairline-b">
        ...
        <div className="flex items-center gap-minor">
          <a href="https://github.com/dylanfernandezdelara/dfdl-ui" target="_blank" rel="noreferrer" className="flex h-8 items-center rounded-sm px-2 text-ui text-fg-secondary hover:bg-surface-hover hover:text-fg transition-interactive duration-fast ease-out">
            GitHub
          </a>
          <ThemeToggle />
        </div>
      ...
      <div className="mx-auto flex w-full max-w-site flex-1">
        ...
        <main className="min-w-0 flex-1 px-major py-major">{children}</main>
      </div>
    </div>
  )
}
...
export function Section({ id, title, lede, children }: { id?: string; title: string; lede?: string; children: ReactNode }) {
  return (
    <section id={id} className="py-12 hairline-t">
